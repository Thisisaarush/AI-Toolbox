      ...
      <Card className="group hover:shadow-md transition-shadow">
        <CardContent className="py-4">
          {item.ogImage && !item.ogImage.startsWith("data:") && (
            ...
            <img src={item.ogImage} alt="" className="w-full h-24 object-cover rounded-xl mb-4" onError={(e) => (e.currentTarget.style.display="none")} />
          )}
              ...
              <div className="flex items-center gap-3 mt-2 flex-wrap">
                ...
                {rt && (
                  <span className="flex items-center gap-1.5 text-xs text-muted-foreground">
                    <Clock className="w-4 h-4" />{rt} min
                  </span>
                )}
          ...
          {/* Actions */}
          <div className="flex items-center gap-1.5 mt-2 pt-2 border-t opacity-0 group-hover:opacity-100 transition-opacity flex-wrap">
            <select
              value={item.status}
            ...
            <Button size="sm" variant="ghost" onClick={() => {
              ...
              {summarizingId === item.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4 mr-1" />}
              {item.aiSummary ? (showSummary ? "Hide" : "Summary") : "Summarize"}
            </Button>
            <Button size="sm" variant="ghost" onClick={() => deleteItem(item.id)} className="text-muted-foreground hover:text-destructive ml-auto">
              <X className="w-4 h-4" />
            </Button>
          </div>
        </CardContent>
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-5xl mx-auto px-4 py-6 w-full space-y-8">
            ...
            <CardContent className="space-y-3">
              {dailyPicks.map((item) => (
                <a key={item.id} href={item.url} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-3 p-3 rounded-xl hover:bg-indigo-500/10 transition-colors">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        {showAddForm && (
          <Card className="border-indigo-500/30">
            <CardContent className="pt-4 space-y-4">
              ...
              {addDesc && <p className="text-xs text-muted-foreground border rounded-xl p-3 bg-muted/30">{addDesc}</p>}
              <div className="flex gap-3">
                <Button size="sm" onClick={addItem}><Check className="w-4 h-4 mr-1" /> Save</Button>
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value as ReadingStatus | "all")} className="h-9 rounded-md border border-input bg-background px-4 text-sm">
            <option value="all">All statuses</option>
            {(["unread","reading","done","archived"] as ReadingStatus[]).map((s) => <option key={s} value={s}>{STATUS_LABELS[s]}</option>)}
          </select>
          <select value={groupBy} onChange={(e) => setGroupBy(e.target.value as GroupBy)} className="h-9 rounded-md border border-input bg-background px-4 text-sm">
            <option value="status">Group by status</option>
            <option value="domain">Group by domain</option>
            <option value="tag">Group by tag</option>
            <option value="readingTime">Group by reading time</option>
          </select>
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value as SortBy)} className="h-9 rounded-md border border-input bg-background px-4 text-sm">
          ...
          <Button size="sm" variant="outline" onClick={() => setShowImport(!showImport)}>
            <BookOpen className="w-4 h-4 mr-1" /> Import
          </Button>
        </div>
        ...
        {showImport && (
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Textarea value={importHtml} onChange={(e) => setImportHtml(e.target.value)} rows={6} placeholder="Paste bookmark HTML here..." className="leading-relaxed" />
              <div className="flex gap-3">
                <Button size="sm" onClick={importBookmarks}><Check className="w-4 h-4 mr-1" /> Import</Button>
        ...
        <Card className="border-dashed">
          <CardContent className="py-4 space-y-4">
            ...
            <Button size="sm" onClick={syncReadwise} disabled={rwLoading || !readwiseToken}>
              {rwLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Zap className="w-4 h-4 mr-1" />Sync</>}
            </Button>
          </CardContent>
        </Card>
        ...
        {store.items.length === 0 ? (
          <div className="py-16 text-center text-muted-foreground">
            <List className="w-14 h-14 mx-auto mb-5 opacity-20" />
            <p className="font-medium text-foreground text-xl">Your reading list is empty</p>
            <p className="text-sm mt-1">Add articles by URL, import bookmarks, or sync from Readwise</p>
            <Button className="mt-4" size="sm" onClick={() => setShowAddForm(true)}><Plus className="w-4 h-4 mr-1" /> Add Article</Button>
          </div>
        ) : filtered.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-3">
              {Array.from(new Set(store.items.flatMap((i) => i.tags))).map((tag) => (
                <button
                  ...
                  className={`px-3 py-1.5 text-xs rounded-full border transition-colors hover:border-indigo-500/50 ${search === tag ? "bg-indigo-500/20 text-indigo-400 border-indigo-500/30" : "border-border text-muted-foreground"}`}
                >
                  {tag}
                </button>
              ))}
        ...
        <div className="bg-muted/30 border border-border rounded-xl p-4 flex items-center gap-3">
          ...
          <p className="text-xs text-muted-foreground">
            <strong>Pro tip:</strong> Click tags to filter by topic. Use the group-by menu to organize by domain, tag, or reading time. Sync from Readwise to import articles you&apos;ve highlighted.
          </p>
        </div>
      </main>
    </div>
