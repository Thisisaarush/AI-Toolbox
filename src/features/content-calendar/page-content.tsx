  ...
  return (
    <div className="space-y-3">
      ...
      <Button size="sm" variant="outline" onClick={() => onChange(value + "\n---\n")}>
        <Plus className="w-4 h-4 mr-1" /> Add tweet
      </Button>
    </div>
  )
}
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-6xl mx-auto px-5 py-6 w-full space-y-8">
        ...
        <div className="flex gap-1.5 p-1 bg-muted/50 rounded-xl flex-wrap">
          {([
            { key: "calendar" as MainTab, label: "Calendar", icon: <Calendar className="w-4 h-4" /> },
            { key: "list" as MainTab,     label: "List",     icon: <List className="w-4 h-4" /> },
            { key: "ideas" as MainTab,    label: "AI Ideas", icon: <Sparkles className="w-4 h-4" /> },
            { key: "repurpose" as MainTab,label: "Repurpose",icon: <Repeat2 className="w-4 h-4" /> },
            { key: "analytics" as MainTab,label: "Analytics",icon: <BarChart3 className="w-4 h-4" /> },
            { key: "pillars" as MainTab,  label: "Pillars",  icon: <span className="w-4 h-4 inline-flex items-center justify-center text-xs">⬡</span> },
            { key: "settings" as MainTab, label: "Settings", icon: <Settings className="w-4 h-4" /> },
          ]).map((t) => (
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={fpPlatform} onChange={(e) => setFpPlatform(e.target.value as Platform)} className="w-full h-9 rounded-md border border-input bg-background px-4 text-sm">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <select value={fpType} onChange={(e) => setFpType(e.target.value as ContentType)} className="w-full h-9 rounded-md border border-input bg-background px-4 text-sm">
                    {CONTENT_TYPES.map((t) => <option key={t}>{t}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-xs font-medium block mb-1">Status</label>
                  <select value={fpStatus} onChange={(e) => setFpStatus(e.target.value as ContentStatus)} className="w-full h-9 rounded-md border border-input bg-background px-4 text-sm">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={fpPillar} onChange={(e) => setFpPillar(e.target.value)} className="w-full h-9 rounded-md border border-input bg-background px-4 text-sm">
              ...
              <div className="flex gap-4 flex-wrap">
                <Button size="sm" onClick={savePiece}><Check className="w-4 h-4 mr-1" /> Save</Button>
                {editingPiece && store.ghostConfig && (
                  <Button size="sm" variant="outline" onClick={() => publishToGhost(editingPiece)} disabled={publishingTo === editingPiece.id}>
                    {publishingTo === editingPiece.id ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : <ExternalLink className="w-4 h-4 mr-1" />}
                    Publish to Ghost
                  </Button>
                )}
        ...
        {tab === "calendar" && (
          <div className="space-y-5">
            <div className="flex items-center justify-between">
              <h2 className="text-2xl font-bold">{MONTH_NAMES[calMonth]} {calYear}</h2>
              <div className="flex gap-1.5">
                <Button variant="outline" size="icon" onClick={() => { if (calMonth === 0) { setCalYear(y=>y-1); setCalMonth(11) } else setCalMonth(m=>m-1) }}>
                  <ChevronLeft className="w-4 h-4" />
                </Button>
                <Button variant="outline" size="sm" onClick={() => { setCalYear(new Date().getFullYear()); setCalMonth(new Date().getMonth()) }}>Today</Button>
                <Button variant="outline" size="icon" onClick={() => { if (calMonth === 11) { setCalYear(y=>y+1); setCalMonth(0) } else setCalMonth(m=>m+1) }}>
                  <ChevronRight className="w-4 h-4" />
                </Button>
              </div>
            </div>
            <div className="grid grid-cols-7 gap-0.5">
              {DOW.map((d) => <div key={d} className="py-2.5 text-center text-xs font-semibold text-muted-foreground">{d}</div>)}
              {calDays.map((day, i) => {
                if (day === null) return <div key={`empty-${i}`} className="min-h-24 rounded-xl" />
                ...
                return (
                  <div
                    ...
                    className={`min-h-24 rounded-xl border p-1.5 cursor-pointer transition-colors hover:bg-muted/30 ${isToday ? "border-fuchsia-500/50 bg-fuchsia-500/5" : "border-border"} ${isSelected ? "ring-2 ring-fuchsia-500" : ""}`}
                  >
                    ...
                    <div className="space-y-0.5">
                      {pieces.slice(0,3).map((p) => (
                        <div key={p.id} className={`text-xs px-1 py-0.5 rounded truncate border ${PLATFORM_COLORS[p.platform]}`}>{p.title}</div>
                      ))}
                      {pieces.length > 3 && <div className="text-xs text-muted-foreground">+{pieces.length-3} more</div>}
                    </div>
                  </div>
                )
              })}
                ...
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-4">
                      <Button size="sm" variant="outline" onClick={() => openForm(undefined, selectedDate)}><Plus className="w-4 h-4 mr-1" /> Add</Button>
                      <Button size="icon" variant="ghost" onClick={() => setSelectedDate(null)}><X className="w-4 h-4" /></Button>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                    ...
                    <div className="space-y-3">
                      {(byDate.get(selectedDate) ?? []).map((p) => (
                        <div key={p.id} className="flex items-center gap-4">
                          ...
                          <Button size="icon" variant="ghost" onClick={() => openForm(p)}><Edit3 className="w-4 h-4" /></Button>
                        </div>
                      ))}
                    </div>
                  )}
        ...
        {tab === "list" && (
          <div className="space-y-5">
            <div className="flex flex-wrap gap-4">
              <select value={filterPlatform} onChange={(e) => setFilterPlatform(e.target.value as Platform | "all")} className="h-9 rounded-md border border-input bg-background px-4 text-sm">
                <option value="all">All platforms</option>
                {PLATFORMS.map((p) => <option key={p}>{p}</option>)}
              </select>
              <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value as ContentStatus | "all")} className="h-9 rounded-md border border-input bg-background px-4 text-sm">
                      ...
                      <CardContent className="py-4">
                        <div className="flex items-center gap-4 flex-wrap">
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            <Button size="icon" variant="ghost" onClick={() => openForm(p)}><Edit3 className="w-4 h-4" /></Button>
                            {ghostUrl && <Button size="icon" variant="ghost" onClick={() => publishToGhost(p)} disabled={publishingTo === p.id}>{publishingTo === p.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <ExternalLink className="w-4 h-4" />}</Button>}
                            <Button size="icon" variant="ghost" onClick={() => deletePiece(p.id)} className="text-muted-foreground hover:text-destructive"><X className="w-4 h-4" /></Button>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
        ...
        {tab === "ideas" && (
          <div className="space-y-5">
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-4"><Sparkles className="w-4 h-4 text-fuchsia-400" />AI Content Ideas</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-3 gap-4">
                  <div>
                ...
                <Button size="sm" onClick={generateIdeas} disabled={ideasLoading}>
                  {ideasLoading ? <><Loader2 className="w-4 h-4 animate-spin mr-1" />Generating...</> : <><Sparkles className="w-4 h-4 mr-1" />Generate 10 Ideas</>}
                </Button>
              </CardContent>
            </Card>
            {ideas.length > 0 && (
        ...
        {tab === "repurpose" && (
          <div className="space-y-5">
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-4"><Repeat2 className="w-4 h-4 text-fuchsia-400" />Repurposing Engine</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                <div>
                ...
                <Button size="sm" onClick={repurpose} disabled={repurposeLoading}>
                  {repurposeLoading ? <><Loader2 className="w-4 h-4 animate-spin mr-1" />Repurposing...</> : <><Repeat2 className="w-4 h-4 mr-1" />Repurpose Content</>}
                </Button>
              </CardContent>
            </Card>
            {repurposed && (
              <div className="space-y-5">
                    ...
                    <CardContent className="space-y-3">
                      {thread.tweets.map((tw, j) => (
                        <div key={j} className="flex gap-4">
                          ...
                          <p className="text-sm flex-1 border rounded-xl p-3 bg-muted/30">{tw}</p>
                      ...
                      <Button size="sm" variant="outline" onClick={() => { setFpTitle(thread.title); setFpPlatform("Twitter/X"); setFpType("thread"); setFpDraft(thread.tweets.join("\n---\n")); setShowForm(true); setTab("list") }}>
                        <Plus className="w-4 h-4 mr-1" /> Save as draft
                      </Button>
                    </CardContent>
                  </Card>
                ))}
                <Card>
                  ...
                  <CardContent>
                    <p className="text-sm whitespace-pre-line border rounded-xl p-4 bg-muted/30">{repurposed.linkedInPost}</p>
                ...
                <Card>
                  ...
                  <CardContent>
                    <p className="text-sm whitespace-pre-line border rounded-xl p-4 bg-muted/30">{repurposed.newsletterSummary}</p>
                  </CardContent>
                </Card>
              </div>
            )}
        ...
        {tab === "analytics" && (
          <div className="space-y-5">
            ...
            {store.pieces.filter((p) => p.status === "published").length === 0 ? (
              <div className="py-12 text-center text-muted-foreground"><BarChart3 className="w-12 h-12 mx-auto mb-4 opacity-20" /><p>No published content yet</p></div>
            ) : (
              <div className="space-y-3">
                    ...
                    <CardContent className="py-4">
                      <div className="flex items-center gap-4">
                        ...
                        <Button size="sm" variant="outline" onClick={() => {
                          ...
                          <BarChart3 className="w-4 h-4 mr-1" /> Log
                        </Button>
                      </div>
                      {analyticsFor === p.id && (
                        <div className="mt-4 grid grid-cols-5 gap-4">
                          ...
                          <div className="col-span-5 flex gap-4">
                            <Button size="sm" onClick={() => saveAnalytics(p.id)}><Check className="w-4 h-4 mr-1" /> Save</Button>
        ...
        {tab === "pillars" && (
          <div className="space-y-5">
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" variant="outline" onClick={() => setShowPillarForm(!showPillarForm)}><Plus className="w-4 h-4 mr-1" /> New Pillar</Button>
            </div>
            {showPillarForm && (
              <Card>
                ...
                {store.pillars.map((pillar) => {
                  ...
                  return (
                    <Card key={pillar.id}>
                      <CardContent className="py-4">
                        <div className="flex items-center gap-4">
                          <div className={`w-10 h-11 rounded-xl flex items-center justify-center text-lg font-black ${PILLAR_COLOR_MAP[pillar.color] ?? ""}`}>
                          ...
                          <Button size="icon" variant="ghost" onClick={() => update((s) => ({ ...s, pillars: s.pillars.filter((p) => p.id !== pillar.id) }))} className="text-muted-foreground hover:text-destructive">
                            <X className="w-4 h-4" />
                          </Button>
                        </div>
                      </CardContent>
