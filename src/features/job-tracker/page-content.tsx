      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 py-6 w-full">
            ...
            {state.applications.length === 0 ? (
              <Card className="max-w-md mx-auto mt-12">
                <CardContent className="py-16 text-center">
                  <Briefcase className="w-14 h-14 mx-auto mb-5 text-muted-foreground opacity-40" />
                  <h2 className="text-2xl font-semibold mb-3">No applications yet</h2>
                  <p className="text-muted-foreground text-sm mb-8">Start tracking your job search.</p>
                            ...
                            >
                              <CardContent className="p-5 space-y-3">
                                ...
                                <div className="flex items-center gap-5 flex-wrap">
                                  {app.workMode && (
                                    <span className="text-xs px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground">{app.workMode}</span>
                                  )}
                                  {app.salaryMin && (
                                  ...
                                  <div className="flex ml-auto">
                                    {[1,2,3,4,5].map((r) => (
                                      <Star key={r} className={`w-4 h-4 ${r <= app.rating ? "fill-amber-400 text-amber-400" : "text-muted/30"}`} />
                                    ))}
                                  </div>
                                </div>
        ...
        {view === "app-detail" && selectedApp && (
          <div className="max-w-2xl mx-auto space-y-5">
            <div className="flex items-start justify-between gap-5">
              <div>
                <h1 className="text-3xl font-bold">{selectedApp.company}</h1>
              ...
              <CardContent className="pt-5 space-y-4">
                <div className="flex items-center gap-5">
                  ...
                  <select
                    ...
                    className="flex-1 h-9 rounded-md border border-input bg-background px-4 text-sm"
                  >
                    {(Object.entries(STATUS_META) as [ApplicationStatus, typeof STATUS_META[ApplicationStatus]][]).map(([k, v]) => (
                      <option key={k} value={k}>{v.label}</option>
                    ))}
                ...
                {[
                  ...
                  <div key={f.label} className="flex gap-5 text-sm">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {addInterview && (
                  <div className="p-5 border rounded-xl space-y-3 bg-muted/30">
                    <div className="grid grid-cols-2 gap-5">
                    ...
                    <div className="flex gap-5">
                      ...
                      <Button size="sm" variant="ghost" onClick={() => setAddInterview(false)}><X className="w-4 h-4" /></Button>
                    </div>
                  </div>
                )}
                {selectedApp.interviews.length === 0 ? (
                  ...
                  selectedApp.interviews.map((iv) => (
                    <div key={iv.id} className="p-5 rounded-xl border">
                      <div className="flex items-center gap-5">
            ...
            {state.githubRepos.length > 0 && (
              <Card>
                <CardHeader><CardTitle className="text-sm flex items-center gap-5"><GitBranch className="w-4 h-4" /> Portfolio Items</CardTitle></CardHeader>
                <CardContent>
                    ...
                    {state.githubRepos.map((repo) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-4 px-2.5 py-1.5 rounded-full border text-xs font-medium transition-colors ${
                            isSelected
        ...
        {view === "resumes" && (
          <div className="space-y-8">
              ...
              <CardContent className="pt-5">
                ...
                {state.githubRepos.length > 0 && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mt-3">
                    {state.githubRepos.map((repo) => (
                      <div key={repo.id} className="p-5 rounded border text-sm flex items-start gap-5">
                        <div className="flex-1 min-w-0">
                          <p className="font-medium truncate">{repo.name}</p>
                          <p className="text-xs text-muted-foreground truncate">{repo.description}</p>
                          {repo.language && <Badge variant="secondary" className="text-xs mt-1">{repo.language}</Badge>}
                        </div>
            ...
            {/* Add resume */}
            <Card>
              <CardHeader><CardTitle className="text-sm flex items-center gap-5"><FileText className="w-4 h-4 text-blue-500" /> Add Resume Version</CardTitle></CardHeader>
              <CardContent className="space-y-4">
